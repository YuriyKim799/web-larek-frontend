...
Функция getProductList() - получачем массив карточек товара

Функция getProductItem(id) - получаем конкретную карточку по id

Функция orderProducts(order) - отправляем на сервер объект заказа и получаем результат
...
В полях класса хранятся следующие данные:
 ...
 previewCard: ICard = null; объект карточки товара, открывается при клике на саму карточку в списке на главной странице.
  ...
  Функция setCards(cards){} наполняет поле items массивом карточек товара с сервера

  функция setPreview(card){} наполняет поле previewCard объектом card для предварительного просмотра карточки

  функция inBasket(card){} проверяет есть ли данный товар уже в корзине

  функция addToBasket(card){} добавляет выбранный товар в массив карточек корзины

  функция removeFromBasket(card){} удаляет товар из массива карточек корзины 

  функция clearBasket(){} очищает корзину

  функция setPayMethod(method){} устанвалвивает в объект заказа способ оплаты

  функция setOrderField(field,value){} заполняем объект заказа значениями из инпутов формы

  функция validateOrder(){} проверяет заполнены ли поля формы заказа
...
- `cards:changed` -  событие генерируемое при загрузке карточек с сервера.
